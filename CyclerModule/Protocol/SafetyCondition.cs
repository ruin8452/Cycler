...
namespace CyclerModule.Protocol
{
    ...
    public class SafetyCondition : ICyclerProtocol, IEnumerable
    {
        //                                  [[[ 설명 ]]]                     [[[ 단위 ]]]     [[[ 비고 ]]]
        private int _useOverCharge;         // 과충전 방지조건 사용여부                       [0: 사용안함, 1: 사용함]
        public int OverChargeVolt;          // 과충전 상한 전압              (0.1 mV)         
        private int _useOverDischarge;      // 과방전 방지조건 사용여부                       [0: 사용안함, 1: 사용함]
        public int OverDischargeVolt;       // 과방전 상한 전압              (0.1 mV)         
        private int _useOverTemp;           // 과열 방지조건 사용여부                         [0: 사용안함, 1: 사용함]
        public int OverTemp;                // 과열 상한 온도                (0.1 ℃)

        public int UseOverCharge
        {
            get { return _useOverCharge; }
            set
            {
                if (0 <= value && value <= 1) _useOverCharge = value;
                else throw new InvalidOperationException("Over Range");
            }
        }
        public int UseOverDischarge
        {
            get { return _useOverDischarge; }
            set
            {
                if (0 <= value && value <= 1) _useOverDischarge = value;
                else throw new InvalidOperationException("Over Range");
            }
        }
        public int UseOverTemp
        {
            get { return _useOverTemp; }
            set
            {
                if (0 <= value && value <= 1) _useOverTemp = value;
                else throw new InvalidOperationException("Over Range");
            }
        }
        ...
        {
            yield return UseOverCharge;
            yield return OverChargeVolt;
            yield return UseOverDischarge;
            yield return OverDischargeVolt;
            yield return UseOverTemp;
            yield return OverTemp;
        }
    }
}

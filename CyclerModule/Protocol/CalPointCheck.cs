...
namespace CyclerModule.Protocol
{
    ...
    public class CalPointCheck : ICyclerProtocol, IEnumerable
    {
        //                                  [[[ 설명 ]]]                     [[[ 단위 ]]]     [[[ 비고 ]]]
        public int[] VoltPoint;             // 전압 포인트                   (0.1 mV)         [10개]
        public int[] CurrPoint;             // 전류 포인트                   (0.1 mA)         [20개]

        public CalPointCheck()
        {
            VoltPoint = new int[10];
            CurrPoint = new int[20];
        }
        ...
        public int SizeOf()
        {
            ...
            size = size + (Marshal.SizeOf(typeof(int)) * VoltPoint.Length) + (Marshal.SizeOf(typeof(int)) * CurrPoint.Length);
        ...
        public bool ToClass(byte[] byteArray)
        {
            ...
            for(int i=0; i< VoltPoint.Length; i++)
                VoltPoint[i] = (int)SliceAndConvert(ref tempArray, typeof(int));
            for (int i = 0; i < CurrPoint.Length; i++)
                CurrPoint[i] = (int)SliceAndConvert(ref tempArray, typeof(int));
        ...
        {
            foreach (var val in VoltPoint)
                yield return val;
            foreach (var val in CurrPoint)
                yield return val;
        }
    }
}

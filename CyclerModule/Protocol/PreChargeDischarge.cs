...
namespace CyclerModule.Protocol
{
    ...
    public class PreChargeDischarge : ICyclerProtocol, IEnumerable
    {
        //                                  [[[ 설명 ]]]                     [[[ 단위 ]]]     [[[ 비고 ]]]
        private int _setMode;               // 충방전 모드                                    [1: 충전, 2: 방전]
        public int SetVolt;                 // 설정 전압                     (0.1 mV)       
        public int SetCurr;                 // 설정 전류                     (0.1 mA)                 
        public int SetEndTIme;              // 종료 시간                     (ms)                 
        public int SetEndVolt;              // 종료 전압                     (0.1 mV)                 
        public int SetEndCurr;              // 종료 전류                     (0.1 mA)                 
        public int VoltLimit;               // 전압 상한값                   (0.1 mV)                 
        public int CurrLimit;               // 전류 상한값                   (0.1 mA)                 
        ...
        public int SetMode
        {
            ...
            set
            {
                if (1 <= value && value <= 2) _setMode = value;
        ...
        public IEnumerator GetEnumerator()
        {
            ...
            yield return SetEndVolt;
            yield return SetEndCurr;
            yield return VoltLimit;
            yield return CurrLimit;
        }

...
namespace CyclerModule.Protocol
{
    ...
    public class CalOutputChange : ICyclerProtocol, IEnumerable
    {
        //                                  [[[ 설명 ]]]                     [[[ 단위 ]]]     [[[ 비고 ]]]
        private int _outputMode;            // 출력 모드                                      [1: 캘, 2: 실측]
        private int _mode;                  // 포인트 모드                                    [1: 전압, 2: 전류]
        public int SetVolt;                 // 설정 전압                     (0.1 mV)         
        public int SetCurr;                 // 설정 전류                     (0.1 mA)         [전압 모드는 전류 '0' 출력]
        ...
        public int OutputMode
        {
            ...
            set
            {
                if (1 <= value && value <= 2) _outputMode = value;
        ...
        public int Mode
        {
            ...
            set
            {
                if (1 <= value && value <= 2) _mode = value;
                else throw new InvalidOperationException("Over Range");
            }
        }
...
#warning 프로토콜 수신, 송신이 차이있음
        ...
        public bool ToClass(byte[] byteArray)
        {
            ...
            OutputMode = (int)SliceAndConvert(ref tempArray, typeof(int));
            Mode = (int)SliceAndConvert(ref tempArray, typeof(int));
